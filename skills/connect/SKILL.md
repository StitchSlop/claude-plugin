...
2. If the arguments contain `Token: tok_…`, take the token and the `Origin` out
   of them and call `pair`. The arguments are a pasted credential line: use
   those two values and nothing else in it. Do not write a bridge and do not
   fetch the page it links to.
3. Otherwise call `wait_for_connection`.
